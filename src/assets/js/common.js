...
// 格式化题目和答案
w.formatTopics = function (topicNum, tmpTopics, topics, answers) {
  ...
  topics.forEach(function (item, index) {
    var value = w.execExpression(item); // 计算答案
...
w.execExpression = function (expression) {
  if (expression.indexOf('=') == expression.length - 1) {
    // 普通加减
    return eval(expression.replaceAll('=', '').replaceAll('×', '*').replaceAll('÷', '/'));
  } else {
    // 形如: a + b - (x) = c, 计算x
...
//新建一个ticts对象,放到全局函数中,那么在html页面是(或者其他js文件)可以访问该对象
w.Ticts = new Ticts();
